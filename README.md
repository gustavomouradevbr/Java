...
Tabela consolidada de todos os enums do repositório para consulta rápida:

| # | Enum | Constantes | Onde é usado | Propósito |
|---|---|---|---|---|
| 1 | `ClientType` | `REGULAR`, `PREMIUM` | `02-logica-negocio-descontos` | **Perfil de cliente** para cálculo de desconto; possui `fromUserInput` para parse seguro de texto. |
| 2 | `OrderStatus` *(moderno)* | `PENDING_PAYMENT`, `PROCESSING`, `SHIPPED`, `DELIVERED` | `05-enums-basico`, `04-restaurante` | **Ciclo de vida de um pedido**; lido via `valueOf` e exibido diretamente pelo `toString` do enum. |
| 3 | `OrderStatus` *(legado)* | `PENDING_PAYMENT`, `PROCESSING`, `SHIPPED`, `DELIVERED` | `05-extra-enum-com-date` | Mesmas constantes usando `java.util.Date`; exercício comparativo com a API moderna. |
| 4 | `WorkerLevel` | `JUNIOR`, `MID_LEVEL`, `SENIOR` | `05-worker-contratos` | **Nível hierárquico** do trabalhador; tipagem forte substitui Strings mágicas no domínio. |
| 5 | `Color` | `BLACK`, `BLUE`, `RED` | `06-metodos-abstratos` | **Atributo visual** de formas geométricas; enum garante que apenas cores válidas sejam atribuídas. |
